...
/*   Updated: 2018/09/04 18:02:36 by rpinoit          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rt.h"

static t_color  throw_ray(t_env *e, t_ray *ray, int rebound)
{
    t_color color;

    ft_bzero(&color, sizeof(t_color));
    if (inter_algo(e->object, ray, e->object_nb) == TRUE)
    {
...
void raytracer_tracer(t_env *e, t_visu *v, t_ray *ray, t_color *c, int x, int y)
{
    ...
    while (row < e->samples)
    {
        ...
        while (col < e->samples)
        {
            ...
            new_color = throw_ray(e, ray, e->rebound);
